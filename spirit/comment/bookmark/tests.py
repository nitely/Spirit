...
class CommentBookmarkModelsTest(TestCase):
    ...
    def test_comment_bookmark_update_or_create_new(self):
        ...
        """
        self.assertFalse(CommentBookmark.objects.filter(user=self.user, topic=self.topic).exists())
        page = 2
        increased = CommentBookmark.increase_or_create(
            user=self.user, topic=self.topic, comment_number=CommentBookmark.page_to_comment_number(page)
        )
    ...
    def test_comment_bookmark_update_or_create_existent(self):
        ...
        """
        CommentBookmark.objects.create(user=self.user, topic=self.topic, comment_number=0)
        page = 2
        increased = CommentBookmark.increase_or_create(
            user=self.user, topic=self.topic, comment_number=CommentBookmark.page_to_comment_number(page)
        )
