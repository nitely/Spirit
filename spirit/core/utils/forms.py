...
class NestedModelChoiceField(forms.ModelChoiceField):
    ...
    def _populate_choices(self):
        ...
        queryset = self.queryset.filter(**kwargs).prefetch_related(
            Prefetch(self.related_name, queryset=self.queryset)
        )

        for parent in queryset:
            choices.append(
                (self.prepare_value(parent), self.label_from_instance(parent))
            )
            choices.extend(
