...
(function () {
  var Match, calculate_operations, consecutive_where, create_index, diff, find_match, find_matching_blocks, html_to_tokens, is_end_of_tag, is_start_of_tag, is_tag, is_whitespace, isnt_tag, op_map, _recursively_find_matching_blocks, render_operations, wrap;

  is_end_of_tag = function is_end_of_tag(char) {
    return char === '>';
  };

  is_start_of_tag = function is_start_of_tag(char) {
    return char === '<';
  };

  is_whitespace = function is_whitespace(char) {
    return /^\s+$/.test(char);
  };

  is_tag = function is_tag(token) {
    return /^\s*<[^>]+>\s*$/.test(token);
  };

  isnt_tag = function isnt_tag(token) {
    return !is_tag(token);
  };

  Match = function Match(start_in_before1, start_in_after1, length1) {
  ...
  html_to_tokens = function html_to_tokens(html) {
  ...
  find_match = function find_match(before_tokens, after_tokens, index_of_before_locations_in_after_tokens, start_in_before, end_in_before, start_in_after, end_in_after) {
  ...
  _recursively_find_matching_blocks = function recursively_find_matching_blocks(before_tokens, after_tokens, index_of_before_locations_in_after_tokens, start_in_before, end_in_before, start_in_after, end_in_after, matching_blocks) {
    var match;
    ...
    if (match != null) {
      if (start_in_before < match.start_in_before && start_in_after < match.start_in_after) {
        _recursively_find_matching_blocks(before_tokens, after_tokens, index_of_before_locations_in_after_tokens, start_in_before, match.start_in_before, start_in_after, match.start_in_after, matching_blocks);
      }

      matching_blocks.push(match);

      if (match.end_in_before <= end_in_before && match.end_in_after <= end_in_after) {
        _recursively_find_matching_blocks(before_tokens, after_tokens, index_of_before_locations_in_after_tokens, match.end_in_before + 1, end_in_before, match.end_in_after + 1, end_in_after, matching_blocks);
      }
    }
  ...
  create_index = function create_index(p) {
  ...
  find_matching_blocks = function find_matching_blocks(before_tokens, after_tokens) {
    ...
    return _recursively_find_matching_blocks(before_tokens, after_tokens, index_of_before_locations_in_after_tokens, 0, before_tokens.length, 0, after_tokens.length, matching_blocks);
  };

  calculate_operations = function calculate_operations(before_tokens, after_tokens) {
    ...
    is_single_whitespace = function is_single_whitespace(op) {
      if (op.action !== 'equal') {
        return false;
      }
  ...
  consecutive_where = function consecutive_where(start, content, predicate) {
  ...
  wrap = function wrap(tag, content) {
    ...
    while (true) {
      ...
      if (non_tags.length !== 0) {
        rendering += "<".concat(tag, " class=\"diff\">").concat(non_tags.join(''), "</").concat(tag, ">");
      }
  ...
  op_map = {
    equal: function equal(op, before_tokens, after_tokens) {
      return before_tokens.slice(op.start_in_before, +op.end_in_before + 1 || 9e9).join('');
    },
    insert: function insert(op, before_tokens, after_tokens) {
      var val;
      val = after_tokens.slice(op.start_in_after, +op.end_in_after + 1 || 9e9);
      return wrap('ins', val);
    },
    delete: function _delete(op, before_tokens, after_tokens) {
      var val;
  ...
  render_operations = function render_operations(before_tokens, after_tokens, operations) {
  ...
  diff = function diff(before, after) {
    var ops;
