...
def _index(request, queryset, template):
    topics = yt_paginate(queryset, per_page=config.topics_per_page, page_number=request.GET.get("page", 1))
    return render(request, template, context={"topics": topics})


def deleted(request):
    # Private topics cant be deleted, closed or pinned so we are ok
    return _index(request, queryset=Topic.objects.filter(is_removed=True), template="spirit/topic/admin/deleted.html")


def closed(request):
    return _index(request, queryset=Topic.objects.filter(is_closed=True), template="spirit/topic/admin/closed.html")
