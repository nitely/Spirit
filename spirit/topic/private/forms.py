...
from ...core.utils.widgets import CIMultipleInput, CITextInput, MultipleInput
...
class TopicForPrivateForm(forms.ModelForm):
    ...
    def category(self):
        ...
        self._category = Category.objects.get(pk=settings.ST_TOPIC_PRIVATE_CATEGORY_PK)
...
class TopicPrivateInviteForm(forms.ModelForm):
    ...
    def __init__(self, topic=None, *args, **kwargs):
        ...
        self.topic = topic
        # Make it dynamic for testing
        self.fields["user"].widget = cx_text_input(attrs={"placeholder": _("username")})
...
class TopicPrivateJoinForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        private = TopicPrivate.objects.filter(user=self.user, topic=self.topic)
