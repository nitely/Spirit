    ...
    def clean_q(self):
        ...
        if len(q) < settings.ST_SEARCH_QUERY_MIN_LEN:
            raise forms.ValidationError(
                _("Your search must contain at least %(length)s characters.")
...
class AdvancedSearchForm(BaseSearchForm):
    category = forms.ModelMultipleChoiceField(
        queryset=Category.objects.visible(), required=False, label=_("Filter by"), widget=forms.CheckboxSelectMultiple
    )

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.fields["category"].label_from_instance = lambda obj: smart_str(obj.title)
        self.fields["q"].widget.attrs.update({"autofocus": ""})
